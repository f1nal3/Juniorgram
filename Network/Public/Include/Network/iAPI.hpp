...
#define APPLY_API_METHODS \
            ...
            std::optional<Network::MessageResult> checkKernelPing                (std::shared_ptr<Network::Connection> clientConnection,   const Message& message) const override;   \
...
namespace Network
{
    class iAPI
    {
        ...
        /**
         * @brief The method for checking of kernel ping.
         * @param const std::shared_ptr<Connection>& client - for connect messages to the client or kernel.
         * @param Message& message - for use the message header functionality.
         */
        virtual std::optional<MessageResult> checkKernelPing(std::shared_ptr<Connection> client, const Message& message) const = 0;
