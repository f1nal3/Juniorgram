...
* @details
* For example: "--serviceport=65001", "--dbport=5432", "--dbname=juniorgram", "--hostaddr=127.0.0.1", "--user=postgres", "--password=postgres".
...
public:
    ...
    */
    ...
    {
        _argParser = argparse::ArgumentParser("Juniorgram", "0.6",
        argparse::default_arguments::none);

        _argParser.set_assign_chars("=");
        _argParser.add_description("Backend service side of juniorgram application. Just have a fun!");
        _argParser.add_epilog("Some parts of the program may be configured by default.");

        _argParser.add_argument("--serviceport")
            .default_value<std::string>("65001")
            .help("Set the port value for service needs")
        ...
        _argParser.add_argument("--dbport")
