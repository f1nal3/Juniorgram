...
std::string AESCipher::encrypt(const std::string& data, const std::string& key)
{
    ...
    StringSource(
        data, true,
        new StreamTransformationFilter(encryptor, new HexEncoder(new StringSink(encryptedData))));
...
std::string AESCipher::decrypt(const std::string& cipherData, const std::string& key)
{
    ...
    StringSource(
        cipherData, true,
        new HexDecoder(new StreamTransformationFilter(decryptor, new StringSink(decryptedData))));
