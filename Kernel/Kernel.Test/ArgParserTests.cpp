...
TEST_CASE("Method for generating an object of the Settings class [ArgParser][Success]")
{
    ...
    Settings                 settingsDefault = Settings()
                                                .setValue(std::make_pair(ParamType::KernelPort,  "65001"     ))
    ...
    {
        REQUIRE(parserDefault.getSettings().get()->getValue(ParamType::KernelPort ) == settingsDefault.getValue(ParamType::KernelPort ));
