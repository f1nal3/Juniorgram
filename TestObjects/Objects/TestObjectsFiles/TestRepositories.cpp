...
std::vector<Models::ReplyInfo> testRepliesRepository::getReplyHistory(std::uint64_t channelID)
{
        ...
        replyHistoryRow.has_value())
    {
        ...
        {
            replyInfo._senderID   = replyResult[0].as<std::uint64_t>();
            replyInfo._msgIdOwner = replyResult[1].as<std::uint64_t>();
            replyInfo._msgID      = replyResult[2].as<std::uint64_t>();
            replyInfo._message    = replyResult[3].as<std::string>();
            replyInfo._userLogin  = replyResult[4].as<std::string>();
...
std::uint64_t testLoginRepository::loginUser(const Models::LoginInfo& loginInfo)
{
    ...
    catch (const std::uint64_t& numericalError)
    {
        Base::Logger::FileLogger::getInstance().log
        (
            numericalError,
