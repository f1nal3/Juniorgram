...
/** 
* @class MockPostgreAdapter
* @brief Adapter class for working with PostgreSQL.
* This is a thread safe singleton.
*/
class MockPostgreAdapter final : public IAdapter
{
private:
    inline static std::mutex                          _staticMutex{};
    inline static std::shared_ptr<MockPostgreAdapter> _instance{};
...
public:
    ...
    */
    static std::shared_ptr<MockPostgreAdapter> Instance(const std::string_view& options = {});

protected:
    MockPostgreAdapter(const std::string_view& options) : _connection{std::make_unique<pqxx::connection>(pqxx::zview(options))} {}

public:
    MockPostgreAdapter(const MockPostgreAdapter& other) = delete;
    MockPostgreAdapter& operator=(const MockPostgreAdapter& other) = delete;

    MockPostgreAdapter(MockPostgreAdapter&& other) = delete;
    MockPostgreAdapter& operator=(MockPostgreAdapter&& other) = delete;

    virtual ~MockPostgreAdapter() {}
