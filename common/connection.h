...
namespace network
{
	...
	private:
		...
		void writeHeader()
		{
			asio::async_write(mSocket, asio::buffer(&mOutcomingMessagesQueue.front().mHeader, sizeof(Message::MessageHeader)),
		...
		void writeBody()
		{
			asio::async_write(mSocket, asio::buffer(mOutcomingMessagesQueue.front().mBody.data(), mOutcomingMessagesQueue.front().mBody.size()),
				[this](std::error_code error, std::size_t length)
				{
					if (!error)
					{
		...
		void readBody()
		{
			asio::async_read(mSocket, asio::buffer(mMessageBuffer.mBody.data(), mMessageBuffer.mBody.size()),
				[this](std::error_code error, std::size_t length)
				{
					if (!error)
					{
    ...
    public:
        Connection(const OwnerType &owner, asio::io_context &contextLink, asio::ip::tcp::socket socket,
                ...
                : mOwner(owner), mContextLink(contextLink), mSocket(std::move(socket)),
        ...
        void connectToServer(const asio::ip::tcp::resolver::results_type &endpoint) {
            if (mOwner == OwnerType::CLIENT) {
                asio::async_connect(mSocket, endpoint, [this](std::error_code ec, asio::ip::tcp::endpoint endpoint) {
                    if (!ec) {
                        readHeader();
                    }
                });
